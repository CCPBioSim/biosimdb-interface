...
def test_validate_direct_mda_error(client):
    with patch("biosimdb_interface.form.validation.Universe", side_effect=Exception("bad format")):
        response = client.post("/webform", data={
            "save": "1",
            "topology": (io.BytesIO(b"fake"), "topol.gro"),
            "trajectory[]": (io.BytesIO(b"fake"), "traj.xtc"),
...
def test_incompatible_files_returns_error(client):
    ...
    with patch("biosimdb_interface.form.webform.validate_with_mdanalysis", return_value="invalid file format"):
        response = client.post("/webform", data={"save": "1"})
        assert response.status_code == 200
...
def test_validation_mda_failure_flashes_error(client):
    with patch("biosimdb_interface.form.webform.validate_with_mdanalysis", return_value="cannot read file"):
        response = client.post("/webform", data={"save": "1"})
